    ...
    if (MHL_RSEN & mhlTxConfig.mhlHpdRSENflags)
    {
        if (MHL_HPD & mhlTxConfig.mhlHpdRSENflags)
        {
            if (QualifyPathEnable())
            {
            	...
            	{
            		pr_info("SiiMhlTxTmdsEnable: Calling SiiMhlTxDrvTmdsControl( true )\n");
	                SiiMhlTxDrvTmdsControl( true );
	            } else {
	            	pr_info("SiiMhlTxTmdsEnable: RAP_CONTENT_ON & mhlTxConfig.rapFlags == false\n");
	            }
            } else {
            	pr_info("SiiMhlTxTmdsEnable: QualifyPathEnable() == false\n");
            }
        } else {
        	pr_info("SiiMhlTxTmdsEnable: MHL_HPD & mhlTxConfig.mhlHpdRSENflags == false\n");
        }
    } else {
    	pr_info("SiiMhlTxTmdsEnable: MHL_RSEN & mhlTxConfig.mhlHpdRSENflags == false\n");
    }
}
